...
use rts_engine::pb::{Delta, EventsStreamRecord, Snapshot, events_stream_record::Record};
...
fn parse_timestamp(timestamp_str: &str) -> Result<i64> {
    ...
    Err(anyhow::anyhow!("Invalid timestamp format: {}. Use Unix timestamp (seconds or ms), 'now', or 'current'", timestamp_str))
}
...
async fn read_snapshot_at_time(
    ...
    if let Ok(redis::Value::Bulk(meta_fields)) = meta {
        ...
        while i + 1 < meta_fields.len() {
            if let (redis::Value::Data(key), redis::Value::Data(value)) = (&meta_fields[i], &meta_fields[i + 1]) {
                    match key.as_slice() {
                        b"tick" => {
        ...
        if let Some(snap_updated_at) = updated_at_ms {
            ...
            if time_diff > 3600_000 { // 1 hour in milliseconds
            ...
            if let redis::Value::Bulk(pair) = entry {
                ...
                if let redis::Value::Bulk(fields) = &pair[1] {
                    ...
                    while i + 1 < fields.len() {
                        if let (redis::Value::Data(name), redis::Value::Data(data)) = (&fields[i], &fields[i + 1]) {
                            if name == b"data" {
...
async fn read_gamestate_at_time(
    ...
    println!("Reading gamestate at time {} (window: {} to {})", timestamp_ms, start_time, end_time);
...
fn write_timepoint_to_files(timepoint: &TimePoint, output_dir: &str, prefix: &str) -> Result<()> {
    ...
    if !timepoint.intents.is_empty() {
        ...
        println!("Wrote {} intents to {}", timepoint.intents.len(), intents_path);
    }
...
async fn main() -> Result<()> {
    ...
    if args.len() < 4 {
        eprintln!("Usage: {} <game_id> <time1> <time2> [output_dir] [time_window_seconds]", args[0]);
    ...
    let time_window_secs: i64 = args.get(5)
