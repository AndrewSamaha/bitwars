...
use sim::{Engine, IntentEnvelope, SimEntityTypeDef, DEFAULT_STOP_RADIUS};
...
// ── M4: Per-entity-type speed test ─────────────────────────────────────────

/// Two entities at the same position, both moving to the same target,
/// but with different entity_type_ids: "fast" (speed=180) and "slow" (speed=30).
/// The fast entity should arrive many ticks before the slow one.
#[test]
fn per_entity_type_speed() {
    let fast_entity = Entity {
        id: 1,
        entity_type_id: "fast".into(),
    ...
    types.insert("fast".to_string(), SimEntityTypeDef { speed: 180.0, stop_radius: 0.75 });
    types.insert("slow".to_string(), SimEntityTypeDef { speed: 30.0, stop_radius: 0.75 });

    let mut engine = Engine::from_snapshot(state).with_entity_types(types);

    // Issue move to (50, 0) for both
    let move1 = make_move(1, 50.0, 0.0, make_cmd_id(50));
    let move2 = make_move(2, 50.0, 0.0, make_cmd_id(51));
    engine.accept(&move1).unwrap();
    engine.accept(&move2).unwrap();

    // Run ticks and track arrival order
    let mut fast_arrival_tick: Option<u64> = None;
    let mut slow_arrival_tick: Option<u64> = None;

    for _ in 0..600 {
        let finished = engine.tick();
        engine.state.tick += 1;
        if finished.contains(&1) && fast_arrival_tick.is_none() {
            fast_arrival_tick = Some(engine.state.tick);
        }
        if finished.contains(&2) && slow_arrival_tick.is_none() {
            slow_arrival_tick = Some(engine.state.tick);
        }
        if fast_arrival_tick.is_some() && slow_arrival_tick.is_some() {
            break;
        }
    }

    let ft = fast_arrival_tick.expect("fast entity should arrive");
    let st = slow_arrival_tick.expect("slow entity should arrive");
    assert!(
        ft < st,
        "fast entity (speed=180) should arrive before slow (speed=30): fast={ft}, slow={st}"
    );
    // Fast is 6x the speed of slow; arrival ticks should be roughly proportional
    // (allowing for stop_radius and overshoot clamping edge effects).
    assert!(
        ft * 4 < st,
        "fast entity should arrive much earlier: fast={ft}, slow={st}"
    );

    // Run stable ticks
    run_ticks(&mut engine, 10);
    assert_eq!(world_hash(&engine), GOLDEN_PER_TYPE_SPEED, "per-entity-type-speed golden hash");
}
