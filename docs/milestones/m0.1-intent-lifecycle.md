...
**Remaining steps**
- **Finish coverage**: emit `BLOCKED` / `CANCELED` transitions and ensure correct `server_tick` semantics in failure paths.
- **Surface telemetry**: wire lifecycle events into UI/observability dashboards for visibility.
