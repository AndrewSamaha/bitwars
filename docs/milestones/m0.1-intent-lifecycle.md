...
### 5) Protocol header (M0.1 scope) [DONE]

- **Startup log**: `services/rts-engine/src/main.rs` emits `protocol header` with `protocol_version=ENGINE_PROTOCOL_MAJOR` and `content_version` sourced from `CONTENT_VERSION` (default `dev`).
- **Protocol stamping**: Lifecycle + delta payloads already carry `protocol_version`; `/api/v1/intent` publishes envelopes populated with the major version.
- **Enforcement**: `services/rts-engine/src/engine/mod.rs::handle_envelope()` rejects intents whose `protocol_version` major ≠ engine major, emitting lifecycle `REJECTED(PROTOCOL_MISMATCH)`.

### 6) Mini-replay tests (`pnpm test:replay:x`) [DONE]

* **Named replay tests**: each test **x** is a defined scenario with a **beginning world state**, a set of **entities**, **predefined intents**, and a **predefined number of ticks**. Invoke as `pnpm test:replay:x` (e.g. `test:replay:two_entities_move`).
* Run the scenario against an in-memory engine (no Redis) and assert the **final world hash**.
...
### 7) Latency probe CLI [DONE]

* Send **100** `Move` intents; report **p50/p95** from submit → **first lifecycle event** (e.g. `ACCEPTED` or `RECEIVED`) that references that intent (matched by `client_cmd_id`).
* Write metrics as JSON under `./.metrics/m0.1/` and print a brief histogram summary to console.

**Status**
- Implemented as `services/rts-engine/src/bin/latency_probe.rs`; run via `pnpm run latency:probe -- <game_id>` (see [docs/tools/latency-probe.md](../tools/latency-probe.md)). Two Redis connections: reader blocks on events stream before publishing; 1 ms spacing between intents to avoid duplicate `client_cmd_id`; JSON written to `./.metrics/m0.1/latency_probe_<timestamp>.json`.

**Rationale (Option B — measure to lifecycle event, not to first delta):** Measuring submit → first *delta* that references an `intent_id` is problematic: (1) If all 100 intents target the **same** entity, they run sequentially, so “first delta” for later intents is dominated by simulation/queue time (entity moving through prior targets), not pipeline latency. (2) Tracing deltas back to specific intents requires deltas to carry `intent_id` (or fragile heuristics). Measuring to the **first lifecycle event** (e.g. `ACCEPTED`) avoids both: lifecycle events already carry `intent_id` and `client_cmd_id`, so correlation is trivial; and ACCEPTED is emitted as soon as the engine ingests the intent from Redis, so the metric is **pipeline latency** (Redis + engine ingest + stream publish), which is what M0.1’s “observability seed” needs. It validates the lifecycle path end-to-end and is simpler to implement than a delta-based probe.
...
* **Determinism**: running a named replay test (`test:replay:x`) yields **identical** final world hash every time (same beginning state + intents + tick count → same hash).
...
### Mini-replay tests (`pnpm test:replay:x`)

* Each **named test x** is a scenario definition: **beginning world state** (or snapshot), **entities**, **predefined intents** (ordered, with accept ticks), and **predefined number of ticks** to run. Tests are server-side (unit/integration); no Redis required.
* Wire script(s) so that `pnpm test:replay:<name>` runs the corresponding scenario (e.g. `test:replay:two_entities_move`). Reuse shared protobuf from `packages/shared/src/gen/` and the in-memory sim engine.
* Implement the **stable serializer** (sorted keys; numeric normalization) and compute `xxh3`. Assert final world hash; on mismatch print minimal diff and exit non-zero.

### Latency probe CLI

* Implement as Node (`scripts/latency-probe.ts`) or Rust (`services/rts-engine/src/bin/latency_probe.rs`).
* Use existing Redis utilities to publish envelopes and listen on the events stream for the **first lifecycle event** (e.g. `RECEIVED` or `ACCEPTED`) that references each intent — match by `client_cmd_id` to the submit timestamp. No need to trace deltas to intents.
