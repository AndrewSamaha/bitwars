...
it.each([
  ...
  const world = { entities: () => [source], getEntityType: (id: string) => id === "collector_solar" ? { collector: { vfx: "solar_proximity" as const } } : id === "worker" ? { collector: { vfx: "mineral_transport" as const } } : undefined };
