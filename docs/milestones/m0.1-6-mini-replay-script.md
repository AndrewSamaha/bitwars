...
The milestone defines **named replay tests** as `pnpm test:replay:x`, where **x** is a scenario name (beginning world state + entities + predefined intents + predefined tick count). The following are the current ways to run the underlying replay/determinism behavior; named `test:replay:x` scripts can be added to wrap these into the documented API.
