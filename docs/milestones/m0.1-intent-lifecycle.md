...
### 4) Wire / Streams (Redis) [IN PROGRESS]

**Status**
- **Snapshot stream**: [DONE] `services/rts-engine/src/io/redis.rs::publish_snapshot()` now writes Redis keys _and_ `XADD`s `rts:match:{id}:snapshots`.
- **Consumer audit**: update any remaining tools or scripts still tailing legacy `deltas:{id}` streams.
* `rts:match:{id}:intents` — client → engine (**IntentEnvelope**, XADD; consumer group on engine).
* `rts:match:{id}:events` — engine → client (**EventsStreamRecord** with a `oneof` type tag: `lifecycle | delta | metric`).
* `rts:match:{id}:snapshots` — periodic world snapshots.
* **On-wire is protobuf**. A dev-only JSON wrapper may be enabled for local debugging; it is **not part of the protocol**.

**Findings (legacy key usage)**
- **`apps/web/src/app/api/v1/intent/route.ts`** still publishes intents to `intents:{gameId}` via `xaddBuffer`.
- **`apps/web/src/app/api/v2/gamestate/stream/route.ts`** tails `deltas:${GAME_ID}` with `XREAD` for SSE delivery.
- **`apps/web/test/read_redis.ts`**, **`apps/web/test/write_and_read_redis.ts`**, and **`apps/web/test/write_and_read_ioredis.ts`** read or write `deltas:{gameId}`.
- **`services/rts-engine/examples/read_deltas.rs`** XRANGEs `deltas:{game_id}`.

**Recommended updates**
- **Intent ingestion (Next.js v1 route)**: switch to `rts:match:${gameId}:intents` or reuse the envelope publisher in `services/rts-engine/src/io/redis.rs`.
- **SSE consumer (`apps/web/src/app/api/v2/gamestate/stream/route.ts`)**: stream from `rts:match:${GAME_ID}:events`, decoding `EventsStreamRecord` and forwarding deltas.
- **Test + utility scripts**: update delta stream references to `rts:match:{id}:events` (and snapshots to `rts:match:{id}:snapshots` where applicable).
- **Rust example `services/rts-engine/examples/read_deltas.rs`**: refactor to read `rts:match:{id}:events` and decode the `Delta` variant.
