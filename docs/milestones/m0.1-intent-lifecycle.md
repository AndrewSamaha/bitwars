...
### 2) Lifecycle states & events [IN PROGRESS]
...
**Remaining steps**
- **Automated validation**: unit tests now cover policy cancellation/blocked logic (`services/rts-engine/src/engine/intent.rs`). Next, build an integration test / replay harness to assert full lifecycle ordering. Dashboards or saved queries remain _out of scope_ for M0.1.
...
### 3) Identifiers & ordering [IN PROGRESS]

**Remaining steps**
- **Tighten validation**: enforce UUIDv7 byte-length parsing on ingest and log structured errors.
- **Improve observability**: extend metrics/logging around dedupe hits, out-of-order drops, and intent acceptance.

* `client_cmd_id` / `intent_id`: **protobuf `bytes`** (UUIDv7). Clients format as hex/base64 **only** for logs/devtools.
* **Dedupe TTL**: store `(player_id, client_cmd_id)` in Redis with **SETEX 5–15 minutes** (configurable) to drop duplicates.
* `client_seq`: per-player incrementing; engine stores **last seen `client_seq`** and **drops older** (emit `REJECTED(OUT_OF_ORDER)`).
* `server_tick`: stamped on **all** deltas and lifecycle events.

### 4) Wire / Streams (Redis) [IN PROGRESS]

**Remaining steps**
- **Snapshot stream**: publish snapshots under `rts:match:{id}:snapshots` and migrate consumers/tests.
